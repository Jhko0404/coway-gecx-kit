# 🏠 GECX Purifier Rental Billing POC Kit 
본 저장소는 Google Cloud의 **GECX (CX Agent Studio)** 플랫폼 상에서 정수기의 스마트 가전 렌탈 요금 분석 생성형 AI 에이전트를 구성하고 배포하기 위한 **독립 실행형 패키지**이자 배포 설명서입니다.
...
*   **[toolsets/](./toolsets/)**: OpenAPI 스키마 정의
    *   [open_api_schema.yaml](./toolsets/open_api_schema.yaml): 정수기 모의 API 연동 규격 스키마
...
3.  GitHub 계정을 연동하고 본 정수기 Repository를 선택합니다.
...
2.  `servers.url` 부분의 주소를 1단계에서 획득한 **Cloud Run 서비스 URL**로 대체하고 저장합니다.
    ```yaml
    servers:
      - url: https://purifier-bill-mock-xxxxxxxx-uc.a.run.app   # 여기에 붙여넣기
    ```
...
2.  설정을 기입합니다:
    *   **Toolset Name**: `purifier_billing`
    *   **Description**: `정수기 고객 렌탈 청구서 조회 API`
...
*   **Linked Toolsets**: `purifier_billing` (호출 가능 오퍼레이션 `getInvoices` 체크)
...
*   **주입할 세션 파라미터 (Session Parameters)**:
    ...
    *   `customer_name` = `홍길동`
    *   `account_id` = `urn:purifier:rental:product:ban:115720204`
    *   `active_language` = `Korean`

### 대화 테스트 스크립트

1.  **시작 인사**:
    *   **에이전트**: *"안녕하세요, 홍길동 고객님. 정수기 대표 고객센터 상담원입니다. 오늘 어떤 도움이 필요하신가요?"*
2.  **렌탈 요금 상세 조회 요청**:
...
3.  **HTTP API 요청/응답 페이로드 추적 (콘솔 직접 검증)**:
    *   도구 로그 내부의 OpenAPI 연동 오퍼레이션(`purifier_billing_getInvoices`) 로그 노드를 클릭합니다.
    *   여기서 **실제 HTTP Request URL**이 Cloud Run 주소(`https://purifier-bill-mock-xxxxxxxx-uc.a.run.app/getInvoices?...`)로 정상 전송되었는지 조회할 수 있습니다.
